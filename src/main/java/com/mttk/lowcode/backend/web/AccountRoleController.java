...
public class AccountRoleController extends AbstractPersistentWithAuthController{
	@Override
	...
	public ResponseEntity<Document> delete(String id) throws Exception {
		...
		List<Document> accounts = template.find(new Query(Criteria.where("roles").is(id)), Document.class, "account");
		for (Document account : accounts) {
			List<String> groups = account.getList("roles", String.class);	
			if (groups != null) {
				if (groups.remove(id)) {
					//
